...
function execContinuition(;
    ...
    function _callback(cmi)
        ...
        ZAOM.updateB!(model)
        ZAOM.updateSaltTransForcing!(model; Q=ghost[:Q][1])
    ...
    if model.env.Q > Q_max || model.env.Q < Q_min
        println("Now Q = $(model.env.Q) is not within range [$(Q_min), $(Q_max)]")
    ...
    Dataset(output_file, "c") do ds
        ...
        for i = 1:scan_steps
            ...
            println(format("The {}-th search. Current Q = {:.2e} Sv. (max Ψb, minΨb) = ({:.3e}, {:3e}) Sv", i, model.env.Q / 1e6, maximum(model.state.Ψb) / 1e6, minimum(model.state.Ψb/1e6)))
...
while true
    ...
    if status == :INIT
        ...
        local init_ΔQ = sign(parsed["scan-dir"]) * 0.01e6
        ...
        ZAOM.updateSaltTransForcing!(model; Q=Q0)
        ...
        ZAOM.updateSaltTransForcing!(model; Q=Q0 + init_ΔQ)
        ZAOM.stepModel_newton!(
            model;
            res_target  = parsed["newton-res-target"],
            min_iter    = parsed["newton-min-iter"],
        ...
        Dataset(next_file_tmp, "c") do ds
            ...
            dqds[1]                = ghost[:dQds][1] ./ parsed["scales"][3]
            ds_exp[1]              = 0.0
        end
